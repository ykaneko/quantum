...
nova_cfg.CONF.import_opt('libvirt_ovs_bridge', 'nova.virt.libvirt.vif')
...
class QuantumFakeVMAgentRyu(
    ...
    def init(self, nova_conf, quantum_conf):
        ...
        # for vif driver
        self.nova_conf.set_override('libvirt_ovs_bridge',
    ...
    def _init_bridge(self):
        ...
        self.int_br = self._ensure_ovs_br(self.nova_conf.libvirt_ovs_bridge)
    ...
    def _init_tunnel(self):
        ...
        if not ip_lib.device_exists(dev_name, self.root_helper):
            ...
            dummy_device = ip_wrapper.add_dummy(dev_name)
        if self.conf.OVS.tunnel_ip:
            # ip address add $tunnel_ip brd '+' scope global dev $dev_name
            dummy_device.addr.add(4, self.conf.OVS.tunnel_ip, '+')
            # ip link set $tunnel_interface up
            dummy_device.link.set_up()

        self._ensure_ovs_br(self.nova_conf.libvirt_ovs_bridge)
